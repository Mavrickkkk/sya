...
<html>
<head>
    <title>sya - verified</title>
    <meta charset="utf-8"/>
    <meta name="description" content="Partie vérifiée de SYA, trouvez vos prochaines illustrations ou single préférés."/>
    <link rel="icon" href="../pics/favicon.png"/>
    <link rel="stylesheet" href="../style/styleVerified.css">
</head>
<body>
<div class="sticky-bar">
    <div class="bar-content">
        <a href="../"><img src="../pics/logo3.png" class="logoBar"></a>
        <ul class="bar-links">
            <li><a href="unmoderate.php" class="links">unmoderate</a></li>
...
<br>
<img src="../pics/verified.jpg" class="verifiedHome">

<?php
if ($type=="illustration"){
    ?>
    <form action="verified.php?type=illustration" method="POST">
        <div class="searchBox">
    ...
    <div class="searchBox">
        ...
        <a class="non-choiceText" href="verified.php?type=single">SINGLES</a>
    </div>
    ...
    <div class="singleContent">
        ...
        if (isset($_POST["name"])) {
            $search = "%" . $_POST["name"] . "%";
            $requestSearch = "SELECT * FROM illustration WHERE type=2 AND month=$month AND name LIKE ?";
...
<?php
    if ($type=="single"){
?>
    <form action="verified.php?type=single" method="POST">
        <div class="searchBox">
            <input class="search" type="text" name="name" placeholder="nom">
        </div>
        <input class="submit" type="submit" value="send">
    ...
    <div class="searchBox">
        <a class="non-choiceText" href="verified.php">ILLUSTRATIONS</a>
...
<div class="singleContent">
    ...
    if (isset($_POST["name"])) {
        $search = "%" . $_POST["name"] . "%";
        $requestSearch = "SELECT * FROM single WHERE type=2 AND month=$month AND name LIKE ?";
        $stmt = mysqli_prepare($idcom, $requestSearch);
        mysqli_stmt_bind_param($stmt, "s", $search);
    } else {
        $requestSearch = "SELECT * FROM single WHERE type=2 AND month=$month LIMIT ? OFFSET ?";
        $stmt = mysqli_prepare($idcom, $requestSearch);
        mysqli_stmt_bind_param($stmt, "ii", $nbrelt, $gap);
    }
...
<?php
    $countRequest = "SELECT COUNT(*) as total FROM single WHERE type=2";
    $countResult = mysqli_query($idcom, $countRequest);
    $countRow = mysqli_fetch_assoc($countResult);
    $totalElements = $countRow['total'];
    $totalPages = ceil($totalElements / $nbrelt);
...
<footer>
    <div class="footer-content">
        ...
        <a>le meilleur de SYA</a>
        <a href="unmoderate.php">clique ici pour en voir plus</a>
    </div>
</footer>
</body>
</html>